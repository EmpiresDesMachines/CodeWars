...
function compare(a, b) {
  const a1 = a.replace(/\./g, ' .').replace(/\#/g, ' #').split(' ').filter(s => s);
  const b1 = b.replace(/\./g, ' .').replace(/\#/g, ' #').split(' ').filter(s => s);

  const aSum = a1.reduce((sum, s) => {
    if (s.match(/^\#/)) sum += 10000;
    if (s.match(/^\./)) sum += 10;
    if (s.match(/^[a-z]/)) sum += 1;
    return sum;
  }, 0);

  const bSum = b1.reduce((sum, s) => {
    if (s.match(/^\#/)) sum += 10000;
    if (s.match(/^\./)) sum += 10;
    if (s.match(/^[a-z]/)) sum += 1;
    return sum;
