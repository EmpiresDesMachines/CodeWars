...
function phone(strng, num) {
  let bookRecord = strng.split("\n").filter((v) => v.includes(num));

  if (bookRecord.length > 1) return `Error => Too many people: ${num}`;
  if (!bookRecord.length) return `Error => Not found: ${num}`;
  bookRecord = bookRecord.join("");

  const regName = /\<([a-z\s\']+)\>/i;
  const [, name] = bookRecord.match(regName);
  bookRecord = bookRecord.replace(regName, "");

  const addr = bookRecord
    .replace(num, "")
    .replace(/[^a-z\d\.\-]/gi, " ")
    .trim()
    .replace(/ +/g, " ");
