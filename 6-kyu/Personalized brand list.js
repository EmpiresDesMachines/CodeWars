...
function sortedBrands(history) {
  ...
  for (let i = 0; i < history.length; i++) {
    if (history[i].brand in data) {
      data[history[i].brand].c++;
    } else {
      data[history[i].brand] = { c: 1, i };
    }
  }

  return [...new Set(history.map((v) => v.brand))].sort(
    (a, b) => data[b].c - data[a].c || data[a].i - data[b].i,
  );
}
