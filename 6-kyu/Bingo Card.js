...
const genRandUniqNum = (min, max, arr) => Math.floor(min + Math.random() * (max + 1 - min));

function getCard(arg) {
  const arr = [];
  const bingo = ['B', 'I', 'N', 'G', 'O'];
  const vals = [0, 15, 30, 45, 60, 75];

  for (let i = 0; i < 25; i++) {
    if (i == 13) continue;
    const step = Math.floor(i / 5);
    const gen = bingo[step] + genRandUniqNum(vals[step] + 1, vals[step + 1], arr);
    if (!arr.includes(gen)) {
      arr.push(gen);
    } else {
      i--;
