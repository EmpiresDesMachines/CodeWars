...
const fireAndFury = function (tweet) {
  if (tweet.indexOf('FURY') === -1 && tweet.indexOf('FIRE') === -1) return 'Fake tweet.';
  if (tweet.match(/[^EFIURY]/)) return 'Fake tweet.';

  const arr = [];
  const match = tweet.match(/fury|fire/gi) || [];
  ...
  for (let i = 0; i < match.length; i++) {
    ...
    for (let j = i + 1; j < match.length; j++) {
      if (match[j] !== current) break;
      else count++;
    }

    if (current === 'FIRE') {
      arr.push(`You ${'and you '.repeat(count)}are fired!`);
    }

    if (current === 'FURY') {
      arr.push(`I am ${'really '.repeat(count)}furious.`);
    }
