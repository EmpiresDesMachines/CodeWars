...
function acronymBuster(string) {
  ...
  const reg = /\b[A-Z]{3,}\b/g;
  string = string.replace(reg, (v) => acr[v] || v);
  const unknown = string.match(reg);
  if (unknown)
    return (
      unknown[0] + ' is an acronym. I do not like acronyms. Please remove them from your email.'
    );
  return string.replace(/(^|\.\s)[a-z]/g, (v) => v.toUpperCase());
}
