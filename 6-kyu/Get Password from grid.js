...
function getPassword(grid, dir) {
  let position = grid.reduce((acc, v, i) => (v.includes('x') ? [i, v.indexOf('x')] : acc), []);
  let pass = '';
  for (let i = 0; i < dir.length; i++) {
    if (dir[i].includes('right')) position[1] += 1;
    if (dir[i].includes('left')) position[1] -= 1;
    if (dir[i].includes('up')) position[0] -= 1;
    if (dir[i].includes('down')) position[0] += 1;
    if (dir[i].includes('T')) pass += grid[position[0]][position[1]];
  }
