...
function winner(deckSteve, deckJosh) {
  ...
  for (let i = 0; i < deckSteve.length; i++) {
    const steve = cards.indexOf(deckSteve[i]);
    const josh = cards.indexOf(deckJosh[i]);
    console.log(deckSteve[i], deckJosh[i]);
    if (steve > josh) steveWins++;
    else if (steve < josh) joshWins++;
  }
