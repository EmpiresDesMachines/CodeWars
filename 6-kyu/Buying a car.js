...
function nbMonths(startPriceOld, startPriceNew, savingperMonth, percentLossByMonth) {
  let sum = startPriceOld;
  let month = 0;

  if (sum >= startPriceNew) return [month, sum - startPriceNew];
  ...
  while (sum < startPriceNew) {
    ...
    sum = startPriceOld + savingperMonth * month;
  }

  return [month, Math.round(sum - startPriceNew)];
}