...
function getWinner(listOfBallots) {
  const voices = {};
  listOfBallots.forEach((v) => (voices[v] = voices[v] + 1 || 1));
  const max = Object.entries(voices).reduce((max, v) => (v[1] > max[1] ? v : max));

  return max[1] > listOfBallots.length / 2 ? max[0] : null;
}
