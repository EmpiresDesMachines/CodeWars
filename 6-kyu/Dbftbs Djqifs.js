...
function encryptor(key, msg) {
  let enc = '';
  for (let i = 0; i < msg.length; i++) {
    if (/[a-z]/i.test(msg[i])) {
      let char = msg[i].charCodeAt() + (key % 26);
      if (char > 90 && msg[i] === msg[i].toUpperCase()) char -= 26;
      if (char < 65 && msg[i] === msg[i].toUpperCase()) char += 26;
      if (char > 121 && msg[i] === msg[i].toLowerCase()) char -= 26;
      if (char < 97 && msg[i] === msg[i].toLowerCase()) char += 26;
