...
const Plugboard = function (wires) {
  wires = wires || '';
  if ([...new Set(wires)].length !== wires.length) throw Error();
  if (wires.length > 20 || wires.length % 2) throw Error();
  this.process = function (wire) {
    const index = wires.indexOf(wire);
    return index == -1 ? wire : (index % 2 === 1) ? wires[index - 1] : wires[index + 1];
  };
};